...
- **All Gmail DOM automation is isolated to one module.** `src/lib/schedule/gmail-recipe.ts` is the *single point of failure* for Gmail UI changes — every selector and the synthetic-event recipe live there and nowhere else. Its header points to the two canonical references: `research/scheduled-send-api-spike.md` (Verification section, preset path) and `research/pick-date-time-probe.md` (custom path + relabel-target dump, run it to rediscover selectors when Gmail breaks). When Schedule Send breaks post-launch, fix it here.
