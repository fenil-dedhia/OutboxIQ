...
Toolchain scaffolded; **PRD §5.1 onboarding is implemented** (first feature). The `extension/` build pipeline (TypeScript + React + Vite + `@crxjs/vite-plugin`, MV3) is set up and verified end-to-end — `npm run build` produces a loadable unpacked extension in `extension/dist/`. Done: a typed `chrome.storage.local` state layer (PRD §7.2 schema) in `src/lib/`, and the onboarding flow as a **3-step** flow (welcome+transparency+consent → timezone → working hours) — note this consolidates the PRD's original 5 steps; PRD §5.1.3 has been updated to match. **PRD §5.2 (compose integration) and §5.3 shell + Quick Options are implemented** (Session 4): the native "Schedule send" item is relabelled and its click intercepted to open OutboxIQ's enhanced modal (rendered in a Shadow DOM, decision #2), with three dynamically-computed Quick Options that produce real native scheduled sends by driving Gmail's own UI via the verified recipe. Still NOT done in §5.3: the **custom "Pick date & time"** path is a deliberate handoff to Gmail's own picker pending probe verification (`research/pick-date-time-probe.md`), and **§5.3.5 Optimize-for-recipient + §5.3.7 fallback** are deferred whole to the next session (need OAuth + People API + Maps proxy). Settings (§5.8) is still a placeholder stub. The `backend/` is untouched (not needed until Unschedule-on-Reply / Maps proxy). The MV3 manifest is generated from a typed `extension/manifest.config.ts` (the old 4-field `manifest.json` stub was superseded and removed); permissions remain minimal/just-in-time — `storage` plus `host_permissions` scoped to `https://mail.google.com/*` only (no `tabs`, no OAuth/`identity` yet). Onboarding launch is owned by the service worker via lifecycle events + a content-script trigger + the toolbar icon. See the Commands section below.
